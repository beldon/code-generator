...
<mapper namespace="${package!}.${className!}">
    <resultMap id="BaseResultMap" dataType="${(data.basePackage)!}${(data.domainPackage)!}.${domainName!}">
...
<#list columnDatas as columnData>
    ...
    <!--普通插入-->
    <insert id="insert" parameterType="${(data.basePackage)!}${(data.domainPackage)!}.${domainName!}">
    ...
    <!--非空插入-->
    <insert id="insertSelective" parameterType="${(data.basePackage)!}${(data.domainPackage)!}.${domainName!}" >
        <selectKey keyProperty="${(primaryData.name)!}" resultType="String" order="BEFORE">
            select  replace(uuid(),'-','')
        </selectKey>
    ...
    <!--更新所有-->
    <update id="updateByPrimaryKey" parameterType="${(data.basePackage)!}${(data.domainPackage)!}.${domainName!}" >
    ...
    <!--更新非空-->
    <update id="updateByPrimaryKeySelective" parameterType="${(data.basePackage)!}${(data.domainPackage)!}.${domainName!}" >
        update ${(table.tableName)!}
        <set >
        <#list columnDatas as columnData>
            <#if columnData.column.columnKey != "PRI">
